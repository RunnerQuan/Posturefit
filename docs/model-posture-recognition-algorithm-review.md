...
| `closeUp` | 5 | 近景，需要鼻子、双肩、双髋；当前不再作为用户可选模式，仅保留旧数据兼容 |
...
| 拍摄模式 | 必需关键点 |
|---|---|
| `closeUp` | 同侧耳朵、同侧肩；当前仅遗留兼容 |
| `halfBody` | 同侧耳朵、同侧肩、同侧髋 |
| `sitting` | 同侧耳朵、同侧肩、同侧髋、同侧膝 |
| `fullBody` | 同侧耳朵、同侧肩、同侧髋、同侧膝、同侧踝 |

侧面视角会按同侧关键点置信度选择可见侧，不再要求远侧肩/髋同时有效。正面全身照还会经过轻量质量门禁，用于拦截身体明显旋转、关键宽度过小或肩髋宽度比例异常的照片。

第三层校验会对当前视角正式输出的问题做关键点兜底：只要某个正式输出项缺少必要点，就把照片标记为失败并提示重拍，而不是把该项跳过后给出“体态良好”的结果。比如正面高低肩必须有左右肩，正面头部偏移必须有头部、双肩和双髋，侧面头前伸必须有同侧耳朵和肩。
...
2. 拍摄模式：全身、半身、坐姿。`closeUp` 近景模式仅作为旧数据兼容保留，不再在拍摄页展示为可选入口。
...
| `closeUp` | 高低肩、头部偏移、头前伸；遗留兼容，不再作为拍摄页可选入口 |
...
单视角和双视角使用同一套阈值感知高斯评分。核心原则是：理想中心值为 100 分；理想中心到“中心-正常边界中点”之间保持 100 分；从该中点到“正常/轻度异常”边界之间从 100 分平滑衰减到约 90 分；进入轻度/中度/严重异常后，继续按分段高斯曲线衰减。

### 8.1 单项评分：阈值感知高斯

`calculateIssueScore` 接收完整体态问题后，会先看严重度：

- `normal`：按“离理想中心的距离”评分，中心到正常边界中点为 100，正常/轻度异常边界约为 90。
- `undetected`：不参与评分平均。
...
- 头前伸：当前 CVA 理想中心为 `50°`，正常边界为 `48°`；`50°` 到 `49°` 保持 100，`48°` 附近约为 90，低于 `48°` 后从正常/轻度边界分段衰减，`45°` 附近约为 50，`40°` 附近约为 20。
- 膝超伸：理想中心近似为 `177°`，靠近正常边界前半段保持 100，`170°` 和 `185°` 两个正常边界约为 90；低于 170 或高于 185 后按对应方向的严重度边界继续衰减。
- 其他偏差型问题：理想中心通常是 `0°` 偏差，中心到 normal 阈值中点保持 100，normal 阈值处约为 90，mild 阈值处约为 50，moderate 阈值处约为 20。

### 8.2 曲线校准

系统用每个问题已有的正常边界和严重度阈值推导分段高斯曲线：中心值为 100，中心到正常边界中点为 100，正常/轻度异常边界为 90，轻度/中度异常边界为 50，中度/严重异常边界为 20。超过严重边界后继续从 20 分向下衰减。这样总分不会惩罚非常接近中立值的正常姿态，同时仍能反映“已接近异常边界”的项目。
