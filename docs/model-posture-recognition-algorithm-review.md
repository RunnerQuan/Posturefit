...
1. 选择同侧可见的耳朵和肩点。
2. 使用 `atan2(abs(shoulder.y - ear.y), abs(ear.x - shoulder.x))` 计算近似 CVA。
3. 返回实际 CVA。数值越大越接近直立，数值越小头前伸倾向越明显。
...
已修复风险：计算函数、分类阈值和高斯评分现在都使用实际 CVA 语义。
...
单视角和双视角使用同一套阈值感知高斯评分。核心原则是：理想中心值为 100 分；从理想中心到“正常/轻度异常”边界之间允许轻微扣分，正常边界约为 90 分；超过正常边界进入轻度/中度/严重异常后，继续按高斯曲线衰减。

### 8.1 单项评分：阈值感知高斯

`calculateIssueScore` 接收完整体态问题后，会先看严重度：

- `normal`：按“离理想中心的距离”评分，中心值为 100，正常边界约为 90。
- `undetected`：不参与评分平均。
- `mild` / `moderate` / `severe`：以正常边界的 90 分为起点，按超过正常边界的距离继续高斯衰减。
...
其中 `deviation` 会按分段语义解释：正常段使用“离理想中心的距离”，异常段使用“离正常边界的超出量”。例如：

- 头前伸：当前 CVA 理想中心和正常边界都为 `50°`，所以 `CVA >= 50°` 仍是 100；低于 50° 后从 90 分起按 `50 - CVA` 衰减。
- 膝超伸：理想中心近似为 `177°`，`170°` 和 `185°` 两个正常边界约为 90；低于 170 或高于 185 后按最近正常边界的距离继续衰减。
- 其他偏差型问题：理想中心通常是 `0°` 偏差，normal 阈值处约为 90，超过 normal 阈值后按超出量衰减。

### 8.2 曲线校准

系统用每个问题已有的正常边界和严重度阈值推导高斯曲线：正常段校准为“中心 100，正常边界 90”；异常段校准为“正常边界 90，严重边界附近约 55”。这样接近正常边界的项目会有轻微扣分，轻度异常不会断崖下跌，中度和严重异常才明显拉低总分。
